...
class LibraryBook extends Book {
  ...
  decreaseQuantityBy(number = 1) {
    if (typeof number === 'number' && !Number.isNaN(number) && number <= this._quantity)
      this._quantity -= number
  }
  ...
  doHaveBook(requestedBook) {
    if (requestedBook instanceof Book) {
      return this.books.some((el) => {
        return el.name === requestedBook.name && el.author === requestedBook.author
      })
    }
...
const book3 = new LibraryBook("Thus Spoke Zarathustra", "Friedrich Nietzsche", 15)
